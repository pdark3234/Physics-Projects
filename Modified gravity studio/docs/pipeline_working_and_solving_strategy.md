...
## 8. Numeric Solve Path

For configurations where full symbolic simplification is slow or does not converge to a compact form, the numeric solve path offers an alternative route.

After the symbolic pipeline run, the frontend exports the field-equation residuals as SymPy-serializable strings. These are sent to `/api/numeric/solve` alongside the domain specification, unknown names, and parameter values.

The backend (`core/numerics/solve.py`) compiles residuals to numpy-compatible functions via `sp.lambdify` and solves the matter unknowns pointwise across the domain using `scipy.optimize` rootfinding. Each grid point is solved independently, so non-convergent points are marked without blocking the rest of the domain.

Key design decisions:

- Residuals are compiled once per unique `(residuals, variable, unknowns, parameters)` tuple via `@lru_cache`. Repeated domain evaluations with the same residual structure do not recompile.
- The lambdify step uses numpy modules for vectorized evaluation where possible, falling back to scalar loops for expressions that numpy cannot broadcast.
- Metric-function expressions (for TOV terms) are also compiled from the symbolic ansatz strings, so numeric TOV balance is fully consistent with the symbolic solve.

The numeric path does not replace the symbolic pipeline. It extends it by providing an efficient post-solve evaluation route for heavy or parameter-sensitive configurations.

---

## 9. Symbolic Plot Path

The symbolic plot path evaluates solved matter expressions and diagnostic quantities over a user-defined 1D domain.

After a symbolic run, the frontend collects expression strings (solved `rho`, `p`, `P_r`, `P_t`, and diagnostic combinations) and sends them to `/api/plot/evaluate` with domain and parameter values.

The backend (`core/plotting/evaluate.py`) lambdifies each expression with numpy and evaluates over the domain grid. Complex or non-finite values are replaced with `None` in the returned series data; per-series warnings are included for expressions that fail entirely.

The plot path handles missing parameter values gracefully: if a solved expression still contains free symbols after parameter substitution, the backend returns an informative error rather than a silent numpy broadcast failure.
